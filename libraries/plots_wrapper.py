...
def get_iplot(integrated_df: DataFrame, labels: Series = None, title: str = None, hover_data: list = None):
    assert (isinstance(integrated_df, DataFrame))
    assert (isinstance(labels, Series) or labels is None)
    assert (isinstance(title, str) or title is None)

    n_components = len(integrated_df.columns)
    integrated_df['PFI'] = [str(label) for label in labels]
